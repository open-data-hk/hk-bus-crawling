...
class HKEta:
    ...
    def getEtas(self, route_id, seq, language):
        ...
        for company_id in co:
            if company_id == "kmb" and "kmb" in stops:
                ...
                _etas.extend(
                    self.mtr(stop_id=stops["mtr"][seq], route=route, bound=bound["mtr"])
                )
    ...
    def kmb(self, stop_id, route, seq, service_type, co, bound):
        ...
        data = list(
            filter(
                lambda e: len(co) > 1
                or service_type == e["service_type"]
                or e["seq"] == seq + 1,
                data,
            )
        )
    ...
    def nlb(self, stop_id, nlb_id):
        try:
            data = requests.post(
                ...
                json={"routeId": nlb_id, "stopId": stop_id, "language": "zh"},
        ...
        for buses in data:
            for bus in buses["bus"]:
                ...
                delta_second = int(
                    ...
                    if bus["arrivalTimeInSecond"] == "108000"
                    else bus["arrivalTimeInSecond"]
                )
                dt = datetime.fromtimestamp(time.time() + delta_second + 8 * 3600)

                ret.append(
                    {
                        "eta": dt.astimezone(timezone.utc).strftime(
                            "%Y-%m-%dT%H:%M:%S+08:00"
                        ),
    ...
    def lightrail(self, stop_id, route, dest):
        platform_list = requests.get(
            "https://rt.data.gov.hk/v1/transport/mtr/lrt/getSchedule?station_id={}".format(
                stop_id[2:]
            )
        ).json()["platform_list"]
        ret = []
        for platform in platform_list:
            ...
            for e in route_list:
                route_no, dest_ch, dest_en, stop, time_en = (
                    e["route_no"],
                    e["dest_ch"],
                    e["dest_en"],
                    e["stop"],
                ...
                if (
                    route == route_no
                    and (dest_ch == dest["zh"] or "Circular" in dest_en)
                    and stop == 0
                ):
                    waitTime = 0
                    if (
                        time_en.lower() == "arriving"
                        or time_en.lower() == "departing"
                        or time_en == "-"
                    ):
                        waitTime = 0
                    else:
    ...
    def gmb(self, gtfs_id, stop_id, bound, seq):
        ...
        data = list(
            filter(
                lambda e: (e["route_seq"] == 1 and bound == "O")
                or (e["route_seq"] == 2 and bound == "I"),
                data,
            )
        )
