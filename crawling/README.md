...
|--------|-------|-------------|
| `parseGtfs.py` | `routeTime.json` | `gtfs.zip`, `gtfs-tc/` (extracted), `gtfs-tc.json` |
| `parseGtfsEn.py` | `routeTime.json` | `gtfs-en.zip`, `gtfs-en/` (extracted), `gtfs-en.json` |

These two can run in parallel with each other.

### Layer 3 — Needs `gtfs-tc.json` and/or `gtfs-en.json`

| Script | Reads | Output files |
|--------|-------|-------------|
| `gmb.py` | `gtfs-tc/calendar.txt`, `gtfs-tc.json` | `routeList.gmb.json`, `stopList.gmb.json` |
| `sunferry.py` | `gtfs-tc.json`, `gtfs-en.json` | `routeList.sunferry.json`, `stopList.sunferry.json` |
| `fortuneferry.py` | `gtfs-tc.json`, `gtfs-en.json` | `routeList.fortuneferry.json`, `stopList.fortuneferry.json` |
| `hkkf.py` | `gtfs-tc.json`, `gtfs-en.json` | `routeList.hkkf.json`, `stopList.hkkf.json` |

All four can run in parallel once Layer 2 completes.

### Layer 4 — Needs `gtfs-tc.json` + all `routeList`/`stopList` from layers 1 & 3

| Script | Reads | Output files |
|--------|-------|-------------|
| `matchGtfs.py` | `gtfs-tc.json`, `routeList.{co}.json`, `stopList.{co}.json` for all 10 operators | `routeFareList.{co}.json`, `routeGtfs.all.json` |
...
### Layer 6 — Needs `.cleansed.json` + `holiday.json` + `gtfs-tc.json`

| Script | Reads | Output files |
|--------|-------|-------------|
| `mergeRoutes.py` | `routeFareList.{co}.cleansed.json`, `stopList.{co}.json`, `holiday.json`, `gtfs-tc.json` | `routeFareList.mergeRoutes.json`, `routeFareList.mergeRoutes.min.json` |
...
- **`sunferry.py`, `fortuneferry.py`, `hkkf.py`** require *both* `gtfs-tc.json` and `gtfs-en.json`, so they must wait for both `parseGtfs.py` and `parseGtfsEn.py`.
